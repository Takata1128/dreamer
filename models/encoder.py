...
class Encoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.cv1 = nn.Conv2d(4, 16, kernel_size=3)
        self.cv2 = nn.Conv2d(16, 32, kernel_size=3)
        self.cv3 = nn.Conv2d(32, 64, kernel_size=3)

    def forward(self, obs):
        hidden = F.relu(self.cv1(obs))
        hidden = F.relu(self.cv2(hidden))
        embedded_obs = F.relu(self.cv3(hidden)).reshape(hidden.size(0), -1)
        return embedded_obs
